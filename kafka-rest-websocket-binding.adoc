== Websocket binding for Kafka-REST
...
In the following sections, the messages used by the websocket binding of kafka-rest operations are described.
All the operations supported by the original kafka-rest are supported using the above websocket binding. 
In addition to these operations, for consuming messages, there are subscribe operations that are specifically supported for the websocket binding.
...
====
where *_consumer_group_* is a consumer group name and *_instance_name_* is a consumer name.

.Sample Request (creating binary consumer "my_instance" in consumer group "my_consumer_group")
----
...
====
where *_consumer_group_* is a consumer group name, *_instance_name_* is a consumer name, and *_topic_* is a topic name.

.Sample Request (subscribing binary topic "test" by consumer "my_instance" in consumer group "my_consumer_group")
----
...
.Sample Request (unsubscribing topic "test" by consumer "my_instance" in consumer group "my_consumer_group")
----
...
.Sample Request (deleting consumer "my_instance" in consumer group "my_consumer_group")
----
...
.Sample Request (creating json consumer "my_json_instance" in consumer group "my_json_consumer_group")
----
...
.Sample Request (subscribing json topic "jsontest" by consumer "my_json_instance" in consumer group "my_json_consumer_group")
----
...
.Sample Request (unsubscribing topic "jsontest" by consumer "my_json_instance" in consumer group "my_json_consumer_group")
----
...
.Sample Request (deleting consumer "my_json_instance" in consumer group "my_json_consumer_group")
----
