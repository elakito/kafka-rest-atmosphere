...
CAUTION: The format described in this document correspond to the behavior implemented for kafka-rest-websocket version 0.0.1. 
This format will be changed to support returning the response as a series of chunked data when the response is writte with a series
of write operations.
...
In addition to those operations suported by kafka-rest, there are subscribe operations that are 
